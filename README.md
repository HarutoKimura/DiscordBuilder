...
The fastest way to experience the complete workflow is through the dedicated
Discord demo server:

**[Join DiscordBuilder on Discord](https://discord.gg/R3dRharhq)**

In the demo channel, run:

```text
/build request: Build an RSVP page for our next community meetup.
```

DiscordBuilder creates a thread, streams Codex's progress, and returns
screenshots and a working preview URL. Reply in the thread to request an edit,
or react 👍 to approve the latest build.

The hosted bot is intentionally limited to the demo server because each build
runs OpenAI Codex and a dedicated Docker sandbox. To run your own instance, see
[Run the complete Discord flow](#run-the-complete-discord-flow). To evaluate
the Codex pipeline without configuring Discord, see
[Fastest way to test: CLI only](#fastest-way-to-test-cli-only).
