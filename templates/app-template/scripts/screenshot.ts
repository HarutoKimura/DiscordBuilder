...
// (The dot separator cannot collide with route slugs: slugs never contain dots.)
  ...
  try {
    ...
    for (const route of routes) {
      ...
      for (const viewport of VIEWPORTS) {
        // Two attempts per route+viewport: the first request compiles the route
        // in dev mode and can outlast the timeout on a cold cache; the retry is
        // then fast.
        let captured = false;
        let lastError = '';
        ...
        if (!captured) {
          ...
          // The route itself is broken — rerunning the same futile retries at
          // the next viewport only burns quality-loop time. Move on.
          break;
        }
      }
    }
